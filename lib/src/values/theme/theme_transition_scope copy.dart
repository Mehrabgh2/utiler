...
class _ThemeTransitionScopeState extends State<ThemeTransitionScope>
  ...
  Map<String, dynamic>? _lastJsonTheme;
  Map<String, dynamic>? _currentJsonTheme;
  ...
  void didChangeDependencies() {
    ...
    if (ValuesScope.isJsonTheme) {
      final manager = ThemeJsonManager.of(context);
      if (manager == null) return;
      _lastJsonTheme = _currentJsonTheme;
      _currentJsonTheme = manager.currentTheme;
      if (_lastJsonTheme == null) {
        _lastJsonTheme = _currentJsonTheme;
        return;
      }
    } else {
    ...
    return Directionality(
      ...
      child: Stack(
        children: [
          widget.child,
          if (_animating)
            ValuesScope.isJsonTheme
                ? ThemeJsonScope(
                    themes: [_isVisible ? _lastJsonTheme! : _currentJsonTheme!],
                    initialTheme: _isVisible
                        ? _lastJsonTheme!.keys.first
                        : _currentJsonTheme!.keys.first,
                    child: widget.child,
                  )
          ...
          if (_animating)
            ValuesScope.isJsonTheme
                ? ThemeJsonScope(
                    themes: [_isVisible ? _currentJsonTheme! : _lastJsonTheme!],
                    initialTheme: _isVisible
                        ? _currentJsonTheme!.keys.first
                        : _lastJsonTheme!.keys.first,
                    child: ClipOval(
